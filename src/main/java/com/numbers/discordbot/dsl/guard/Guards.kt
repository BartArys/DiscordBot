...
inline val IChannel.canReact: Boolean  get() {
    return this.getModifiedPermissions(client.ourUser).contains(Permissions.MANAGE_MESSAGES)
}

inline val IChannel.canMessage: Boolean  get() {
    return this.getModifiedPermissions(client.ourUser).contains(Permissions.SEND_MESSAGES)
}

inline val IChannel.canSendFiles : Boolean get() {
    return this.getModifiedPermissions(client.ourUser).contains(Permissions.ATTACH_FILES)
}

inline val IChannel.canDeleteMessage : Boolean get() {
    return this.getModifiedPermissions(client.ourUser).contains(Permissions.MANAGE_MESSAGES)
}

inline val IVoiceChannel.canJoin : Boolean get() {
    return this.getModifiedPermissions(client.ourUser).contains(Permissions.VOICE_CONNECT) && !this.isFull
}

inline val IVoiceChannel.canSpeak : Boolean get() {
