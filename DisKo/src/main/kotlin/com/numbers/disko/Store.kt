...
inline fun <T> Store<T>.getOrCreateEntity(clazz: Class<T>, key: Long, supplier: () -> T): T where T : Any = getEntity(clazz, key)
        ?: supplier().also { storeEntity(key, it) }

inline operator fun <reified T> Store<T>.invoke(key: Long): T? where T : Any = getEntity(T::class.java, key)
inline operator fun <reified T> Store<T>.invoke(key: Long, supplier: () -> T): T? where T : Any = getOrCreateEntity(T::class.java, key, supplier)
